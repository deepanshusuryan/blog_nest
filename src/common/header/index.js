        ...
        <header className={`header${scrolled ? " header--scrolled" : ""}`}>
            <div className="header-inner">
                ...
                <div className="header-right" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
                    ...
                    <button
                        className={`header-hamburger${mobileMenuOpen ? " header-hamburger--open" : ""}`}
            ...
            {mobileMenuOpen && (
                <div className="header-mobile-menu">
                    ...
                    {user ? (
                        <>
                            <Link href={`/nest/profile/${user?._id}`} className="header-mobile-link" onClick={() => setMobileMenuOpen(false)}>Profile</Link>
                            {user?.role === "super_admin" && (
                                <Link href="/admin" className="header-mobile-link" onClick={() => setMobileMenuOpen(false)}>Admin Panel</Link>
                            )}
                            <button className="header-mobile-link header-mobile-link--danger" onClick={() => { handleLogout(); setMobileMenuOpen(false); }}>Logout</button>
                        </>
                    ) : (
                        <>
                            <Link href="/login" className="header-mobile-link" onClick={() => setMobileMenuOpen(false)}>Sign in</Link>
                            <Link href="/signup" className="header-mobile-btn" onClick={() => setMobileMenuOpen(false)}>Get started</Link>
                        </>
                    )}
                </div>
            )}
